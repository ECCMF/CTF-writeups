...
## Exploit
### SSRF
Using the internal api (the `/file.php` endpoint), we test the endpoint by fetching the *index.php*, which is publicly available.
On Apache-based web server files root is by default in */var/www/html* we try
`http://$IP/file.php?cv=file:///var/www/html/index.php`.
![index fetched](./img/index_php.png)
 This api call is successful without authentication, hence the SSRF is also exposed via broken access control. Index shows that *config.php* is located in the same folder, so we fetch it analogously by replacing *index.php* by *config.php*.
...
Do not allow arbitrary user controled URIs.
Map CVs server-side. For file paths, use cannonicalization and allowlist to enforce files from specified locations only.
Do not store credentials in plaintext in accessible files, especially not during deployment.

### SQL injection
Upon login we are redirected to the dashboard, where we can search for candidates. We test if the search field is vulnerable to SQLi submitting the `'` character which yields a SQL syntax error, suggesting that we can use SQLi.
Now we confirm and enumerate the columns of the request using the union select and increasing the column count starting with one column until we get to the correct numbe of columns - four:
`'UNION SELECT database(), 2, 3, 4 -- `
![sqli enum](./img/sqli_enum.png)
Now that we know how many columns there are and what is the DB name, we can enumerate further to learn what tables exist:
...
#### Remediation
Use parametrised queries, input sanitisation functions and prepared queries for user supplied input.

## Summary
Through enumeration we were able to recover the location of user account credentials (the HR account). Through SSRF we have recovered those credentials and used them to establish a foothold in the application. After that, we have discovered the SQLi vulnerability, which we used to obtain the admin account. This chain led us from unauthenticated user to admin.
